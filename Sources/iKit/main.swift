...
class Daemon {
  ...
  private func runLoop(outputDir: String, fm: FileManager) async {
    ...
    // Spawn as detached process so it survives daemon exit
    if !pendingTranscriptions.isEmpty {
      let pending = pendingTranscriptions
      pendingTranscriptions.removeAll()
      spawnDetachedTranscription(pending)
    }
  }

  /// Spawn transcription as an independent process (new session) that survives daemon exit.
  /// Writes a manifest file, then launches a shell script that transcribes each file sequentially.
  private func spawnDetachedTranscription(_ pending: [([URL], String)]) {
    let configManager = ConfigManager.shared

    guard configManager.getMeetAutoTranscribe() else {
      Logger.info("⏭️  Auto-transcription disabled in config")
      return
    }
    ...
    guard let python = configManager.current.python_path,
      ...
      Logger.warn("⚠️  Python/Script not configured, skipping post-stop transcription")
      return
    }

    // Collect all audio files to transcribe
    var audioFiles: [(file: URL, output: URL)] = []
    for (files, outputDir) in pending {
      for file in files {
        let out = file.deletingPathExtension().appendingPathExtension("json")
        if !FileManager.default.fileExists(atPath: out.path) {
          audioFiles.append((file, out))
        }
      }
    }

    guard !audioFiles.isEmpty else {
      Logger.info("✅ All segments already transcribed")
      return
    }

    Logger.info("🎤 Spawning detached transcription for \(audioFiles.count) file(s)...")

    // Build a shell script that transcribes each file sequentially
    let outputDir = pending.first?.1 ?? ""
    var scriptLines = [
      "#!/bin/bash",
      "# Auto-generated by iKit after_stop flush",
      "# This runs independently of the daemon process",
      "",
    ]
    for (file, out) in audioFiles {
      // Each file gets its own invocation with error handling
      scriptLines.append("echo \"[$(date '+%Y-%m-%d %H:%M:%S')] 🎤 Transcribing: \(file.lastPathComponent)\"")
      scriptLines.append("\"\(python)\" \"\(script)\" \"\(file.path)\" --output \"\(out.path)\" 2>&1 && \\")
      scriptLines.append("  echo \"[$(date '+%Y-%m-%d %H:%M:%S')] ✅ Done: \(out.lastPathComponent)\" || \\")
      scriptLines.append("  echo \"[$(date '+%Y-%m-%d %H:%M:%S')] ❌ Failed: \(file.lastPathComponent)\"")
      scriptLines.append("")
    }
    scriptLines.append("echo \"[$(date '+%Y-%m-%d %H:%M:%S')] 🏁 All transcriptions complete\"")

    // Write the script to a temp file in the output directory
    let scriptPath = URL(fileURLWithPath: outputDir).appendingPathComponent(".transcribe-flush.sh")
    let logPath = URL(fileURLWithPath: outputDir).appendingPathComponent(".transcribe-flush.log")
    do {
      try scriptLines.joined(separator: "\n").write(to: scriptPath, atomically: true, encoding: .utf8)
      try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: scriptPath.path)
    } catch {
      Logger.warn("⚠️  Failed to write transcription script: \(error)")
      return
    }

    // Spawn detached: nohup + & ensures process survives daemon exit on macOS
    let task = Process()
    task.executableURL = URL(fileURLWithPath: "/bin/bash")
    task.arguments = ["-c", "nohup bash \"\(scriptPath.path)\" > \"\(logPath.path)\" 2>&1 &"]

    do {
      try task.run()
      Logger.info("🚀 Detached transcription launched (log: \(logPath.lastPathComponent))")
      Logger.info("   Check progress: cat \"\(logPath.path)\"")
    } catch {
      Logger.warn("⚠️  Failed to spawn detached transcription: \(error)")
    }
  }

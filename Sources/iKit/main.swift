...
class Daemon {
    ...
    func run(outputDir: String) async {
        ...
        Logger.info("Press Ctrl+C to stop recording and save files")
    ...
    private func runLoop(outputDir: String, fm: FileManager) async {
        ...
        while !Task.isCancelled {
            ...
            // 15 mins block (with cancellation support)
            do {
...
class CalendarTool {
    ...
    func newEvent(title: String, time: String) async {
        ...
        event.title = title; event.calendar = store.defaultCalendarForNewReminders()
...
struct App {
    ...
    static func printHelp(for command: String?) {
        let helpText: String
        switch command {
        case "task": helpText = "Task: list [--json], new <title>, complete <query> [--id], delete <query> [--id] [--dry-run]"
